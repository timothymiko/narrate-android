...
public abstract class MonthView extends View {
    ...
    protected static int CIRCLE_INDICATOR_SiZE;
    ...
    protected Paint mCircleIndicatorPaint;
    ...
    private boolean mDrawTitles = true;
    private boolean mFixedHeightFlag;
    private int mFixedHeight = -1;

    private boolean mAnimatingTitles;
    private float mTargetAlpha;
    private long mStartTime;
    private long DEFAULT_ANIM_DURATION = 200l;
    private Runnable mTitleAnimRunnable = new Runnable() {
        @Override
        public void run() {
            long elapsed = System.currentTimeMillis() - mStartTime;

            if ( elapsed < DEFAULT_ANIM_DURATION ) {

                mAnimatingTitles = true;

                float fraction = (float)elapsed / (float)DEFAULT_ANIM_DURATION;

                if ( mTargetAlpha == 0 )
                    fraction = 1.0f - fraction;

                float alpha = fraction * 255f;
                setTitleAlpha(alpha);

                if ( getHandler() != null )
                    getHandler().postDelayed(this, 45);

            } else {
                mAnimatingTitles = false;
                setTitleAlpha(mTargetAlpha);
            }
        }
    };
    ...
    public void setSelectedColor(int color) {
        mSelectedCirclePaint.setAlpha(0);
        mSelectedCirclePaint.setColor(color);
        mCircleIndicatorPaint.setColor(color);
    }

    public void animateTitles(boolean hide) {
        mTargetAlpha = hide ? 0 : 255f;

        // adjust start time to allow animation to run for enough time to finish
        // alpha animation
        float alphaToCover = Math.abs(mMonthTitlePaint.getAlpha()-mTargetAlpha);

        if ( alphaToCover > 0 ) {
            double fraction = alphaToCover / 255.0;
            long timeToCover = Math.round(fraction * (double) DEFAULT_ANIM_DURATION);
            long offset = DEFAULT_ANIM_DURATION - timeToCover;
            mStartTime = System.currentTimeMillis() - offset;

            if (!mAnimatingTitles)
                post(mTitleAnimRunnable);

            mAnimatingTitles = true;
        } else {
            removeCallbacks(mTitleAnimRunnable);
            mAnimatingTitles = false;
        }
    ...
    protected void initView() {
        ...
        mCircleIndicatorPaint = new Paint();
        mCircleIndicatorPaint.setAntiAlias(true);
        mCircleIndicatorPaint.setStyle(Style.FILL);
        mCircleIndicatorPaint.setColor(mDayTextColor);
    }

    public void setTitleAlpha(float alpha) {
        mMonthTitlePaint.setAlpha((int)alpha);
        invalidate();
    }

    public float getTitleAlpha() {
        return mMonthTitlePaint.getAlpha();
    }
    ...
    public void setDrawTitles(boolean draw) {
        this.mDrawTitles = draw;
        invalidate();
    }

    @Override
    protected void onDraw(Canvas canvas) {
        if (mDrawTitles) drawMonthTitle(canvas);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (mFixedHeightFlag) {
            setMeasuredDimension(MeasureSpec.getSize(widthMeasureSpec), mFixedHeight);
